    ...
    for user in users:
        ...
        if last_vps_payment['status'] != "Pagar":
            ...
            if today >= last_vps_payment['expiration']:
                ...
                message = f'<div style:margin: 0em auto; border: 0.15em solid #000; border-radius: 0.3em;"><a href="http://127.0.0.1:8000/" target="_blank"><img src="/static/img/logo.png" alt="logo"></a><h1>Hola {user[1]} {user[2]}</h1><p style="font-size: 1.2em;">Se le informa que ya se puede hacer el pago de la mensualidad de <b>VPS + COPYTRADING.</b> Le informamos también que de no hacerse el pago después de 5 días de la fecha de expiración se desconectara su cuenta de metatrader 5 del sistema copytrading y se reconectara una vez este al día con todos los pagos, esto incluye los pagos de <b>GESTIÓN DE CUENTAS DE MT5.</b></p><a style="padding:0.5em 1em;background:#00f; border: 0.2em solid #D4FF00;color: #fff;text-decoration: none;cursor: pointer;" href="http://127.0.0.1:8000/payments/" target="_blank">Ir a la pagina de pago</a></div>'
    ...
    for user in users:
        ...
        if last_payment != None:
            if last_management['id'] != last_payment['id_management']:
                ...
                message = f'<div style:margin: 0em auto; border: 0.15em solid #000; border-radius: 0.3em;"><a href="http://127.0.0.1:8000/" target="_blank"><img src="/static/img/logo.png" alt="logo"></a><h1>Hola {user[1]} {user[2]}</h1><p style="font-size: 1.2em;">Se le informa que ya se puede hacer el pago de <b>GESTION DE CUENTAS DE METATRADER 5</b>. Le informamos también que de acumularse dos pagos adeudados se desconectara su cuenta metatrader 5 del sistema copytrading y se reconectara una vez este al día con todos los pagos, esto incluye los pagos de <b>VPS + COPYTRADING.</b></p><a style="padding:0.5em 1em;background:#00f; border: 0.2em solid #D4FF00;color: #fff;text-decoration: none;cursor: pointer;" href="http://127.0.0.1:8000/payments/" target="_blank">Ir a la pagina de pago</a></div>'
                send_email(user[3], message=message, affair=affair)
